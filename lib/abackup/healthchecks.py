...
def notify_or_log_error(
    ...
    error_message = "Failed to perform healthcheck for {} - code: {} message: {}".format(
        name, response_code, response_message
    )
    do_notify = notify_mode == notifications.Mode.ALWAYS or notify_mode == notifications.Mode.AUTO

    if not notifier:
        log.debug(
            "notify_or_log_error({}, {}): skipping notify (because no notifier was supplied)".format(
        ...
        if response.is_error():
            ...
            log.debug(
                "notify_or_log_error({}, {}): notify successful: code: {} message: {}".format(
                    name, notify_mode.name, response.code, response.message
                )
            )
...
class Healthcheck:
    def __init__(
        ...
        base_url: str = None,
        uuid: str = None,
        do_include_messages: bool = None,
        do_notify_start: bool = None,
        do_notify_failure: bool = None,
        do_notify_success: bool = None,
    ):
    ...
    def override(self, other):
        return Healthcheck(
            ...
            do_include_messages=other.do_include_messages
            if other.do_include_messages is not None
            else self.do_include_messages,
            do_notify_start=other.do_notify_start if other.do_notify_start is not None else self.do_notify_start,
            do_notify_failure=other.do_notify_failure
            if other.do_notify_failure is not None
            else self.do_notify_failure,
            do_notify_success=other.do_notify_success
            if other.do_notify_success is not None
            else self.do_notify_success,
        )
...
def perform_healthcheck_start(
    default_check: Healthcheck,
    check: Healthcheck,
    name: str,
    notifier: notifications.SlackNotifier,
    notify_mode: notifications.Mode,
