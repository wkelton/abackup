...
def notify_or_log_error(notifier: notifications.SlackNotifier, name: str, notify_mode: notifications.Mode,
    log: logging.Logger, frameinfo: NamedTuple, response_code: int, response_message: str):
    ...
    if not notifier:
        log.debug("notify_or_log_error({}, {}): skipping notify (because no notifier was supplied)"\
            .format(name, notify_mode.name))
        ...
        if response.is_error():
            ...
            log.debug("notify_or_log_error({}, {}): notify successful: code: {} message: {}"\
                .format(name, notify_mode.name, response.code, response.message))
...
def perform_healthcheck_start(default_check: Healthcheck, check: Healthcheck, name: str,
    ...
    if result.is_error():
        notify_or_log_error(notifier, name, notify_mode, log, frameinfo=getframeinfo(currentframe()),
...
def perform_healthcheck(default_check: Healthcheck, check: Healthcheck, name: str,
    notifier: notifications.SlackNotifier, notify_mode: notifications.Mode, log: logging.Logger, is_fail: bool = False,
    message: str = None):
    if is_fail:
