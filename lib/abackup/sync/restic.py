...
class ResticInfo(syncinfo.SyncInfo):
    def __init__(
        ...
        transfer_info: Dict[str, Any] = None,
    ):
        if not timestamp:
        ...
        super().__init__(
            ...
            timestamp,
            duration,
            location_info={"repo_name": repo_name, "path": path},
    ...
    def update_with_result(self, result: ResticResult, info: Dict[str, Any] = None):
        ...
        if not result.succeeded:
            ...
            self.transfer_info[result.command + "_info"] = {"stderr": result.stderr, "stdout": result.stdout}
            return False
        else:
    ...
    def update_with_backup_result(self, result: BackupResult):
        ...
        return self.update_with_result(
            result,
            {
...
def do_restic(
    ...
    global_options: ResticGlobalOptions,
    ...
    log.debug(
        "do_restic({}, {}, #commands:{}, {}, {}, {})".format(
            repo_name, global_options.global_options, len(commands), data_name, path, sync_type
        )
    )
    if len(commands) == 0:
    ...
    for command in commands:
        ...
        if isinstance(command, ResticBackupCommand):
            ...
            result = command.run(restic_wrapper, log, global_options.global_options, args=[path])
            sync_info.update_with_backup_result(result)
        elif isinstance(command, ResticCheckCommand):
            result = command.run(restic_wrapper, log, global_options.global_options)
...
def do_auto_restic(
    ...
    if do_healthchecks and auto_sync.healthchecks:
        hc.perform_healthcheck_start(
            config.default_healthcheck, auto_sync.healthchecks, repo_name, config.notifier, notify_mode, log
        )

    log.info("running restic with {} repo on {}".format(repo_name, data_dir.path))

    ret = do_restic(
        repo_name,
        repo,
        data_dir.restic_options.mask(auto_sync.driver.settings.global_options),
        auto_sync.driver.commands,
        data_name,
        data_dir.path,
        log,
        sync_type,
    ...
    error_message = None
    if not ret:
        error_message = "Failed syncing with {}!".format(auto_sync.driver.settings.repo_name)
        syncinfo.handle_failed_sync(config.notifier, data_name, repo_name, False, error_message, notify_mode, log)
    else:
        syncinfo.handle_sync_results(config.notifier, data_name, repo_name, False, ret, notify_mode, log)
        sync_info = ret
