            ...
            for info in sync_infos:
                if not info.pull:
                    rows.append([name, "{:7}: {}".format(info.sync_type, info.timestamp),
                                 "{}: {}".format(info.remote_host,
                                                 info.destination) if info.remote_host else info.destination,
                                 "{} (-{})".format(info.sync_count, info.sync_deleted),
                                 fs.to_human_readable(info.sync_bytes),
                                 info.duration])
        else:
