...
def perform_examine(config: Config, cron: appcron.AppCronTab, log: logging.Logger):
    # print owned_data
    rows = [[name, data_dir.path, fs.to_human_readable(fs.get_total_size(data_dir.path, log), prefix='Ki')]
                ...
                if 'pull' in info.location_info:
                    if not info.location_info['pull']:
                        rows.append([name, "{:7}: {}".format(info.sync_type, info.timestamp),
                                     "{}: {}".format(info.location_info['remote_host'],
                                                     info.location_info['destination']) if info.location_info['remote_host'] else info.location_info['destination'],
                                     "{} (-{})".format(
                                         info.transfer_info['sync_count'], info.transfer_info['sync_deleted']),
                    ...
                    rows.append([name, "{:7}: {}".format(info.sync_type, info.timestamp),
                                 "{}: {}".format(info.location_info['repo_name'],
                                                 info.location_info['path']),
                                 files, data_added, info.duration])
        else:
            ...
            for info in sync_infos:
                if 'pull' in info.location_info:
                    if info.location_info['pull']:
                        rows.append([name, "{:7}: {}".format(info.sync_type, info.timestamp),
                                     "{}: {}".format(info.location_info['remote_host'],
                                                     info.location_info['origin']) if info.location_info['remote_host'] else info.location_info['origin'],
                                     "{} (-{})".format(
                                         info.transfer_info['sync_count'], info.transfer_info['sync_deleted']),
                                     info.transfer_info['sync_bytes'],
                                     info.duration])
