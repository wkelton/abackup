...
def perform_rsync(
    ...
    if remote:
        ...
        log.info(
            "syncing {} with {}".format(
                "{}:{}".format(remote.host, origin) if pull else origin,
    ...
    ret = do_rsync(
        origin, destination, data_dir.rsync_options.mask(RsyncOptions(delete, max_delete)), log, remote, pull=pull
    )
...
def perform_auto_sync(
    ...
    notify: str,
    log: logging.Logger,
    only_data_name: str = None,
    only_sync_name: str = None,
    ...
    def process_auto_sync(auto_sync: AutoSync, is_stored_data: bool):
        if isinstance(auto_sync.driver, RsyncDriver):
            log.debug("Performing auto rsync on {}".format(auto_sync.sync_name))
            return do_auto_rsync(
                config,
                name,
                data_dir,
                auto_sync,
                ...
                sync_type,
                pull=is_stored_data,
                do_healthchecks=do_healthchecks,
            )
        elif isinstance(auto_sync.driver, ResticDriver):
            if is_stored_data:
                log.warning("restic driver not supported in stored_data: {}".format(auto_sync.sync_name))
            else:
                log.debug("Performing auto restic on {}".format(auto_sync.sync_name))
                return do_auto_restic(config, name, data_dir, auto_sync, notify, log, sync_type, do_healthchecks)
        else:
            log.error(
                "unrecognized sync driver: {} for: {}".format(auto_sync.driver.__class__.__name__, auto_sync.sync_name)
            )
        return None
    ...
    for name, data_dir in config.owned_data.items():
        sync_succeeded = process_data_dir(name, data_dir, is_stored_data=False) and sync_succeeded

    for name, data_dir in config.stored_data.items():
        sync_succeeded = process_data_dir(name, data_dir, is_stored_data=True) and sync_succeeded
