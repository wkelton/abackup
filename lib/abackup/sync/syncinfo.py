...
class SyncInfo:
    def __init__(self, name: str, sync_type: str, timestamp: datetime.datetime, duration: datetime.timedelta,
                 location_info: Dict[str, Any], transfer_info: Dict[str, Any]):
        self.name = name
...
def flatten_sync_info(sync_info: SyncInfo, pretty: bool = True):
    m = {'name': sync_info.name, 'sync_type': sync_info.sync_type,
         'timestamp': sync_info.timestamp.replace(microsecond=0).isoformat(), 'duration': str(sync_info.duration)}
    if pretty:
        m = {'Name': sync_info.name, 'Sync Type': sync_info.sync_type,
             'Timestamp': sync_info.timestamp.replace(microsecond=0).isoformat(), 'Duration': str(sync_info.duration)}
...
def unjsonify_sync_info(json_dict: Dict[str, Any]):
    ...
    return SyncInfo(_get_field('name'),
                    _get_field('sync_type'),
                    datetime.datetime.strptime(_get_field(
...
class SyncInfosSerializer:
    def __init__(self, config: Config, data_name: str):
        self.json_file_path = os.path.join(
            config.log_root, "{}-latest.json".format(data_name))
    ...
    def serialize(self, sync_infos: List[SyncInfo]):
        try:
            ...
            if not previous_sync_infos:
                previous_sync_infos = []
            sync_infos_dict = {
                sync_info.name: sync_info for sync_info in previous_sync_infos}
            for sync_info in sync_infos:
                sync_infos_dict[sync_info.name] = sync_info
            with open(self.json_file_path, 'w') as json_file:
                print(serialize_sync_infos(
                    sync_infos_dict.values()), file=json_file)
            return True
        except TypeError:
            return False
        except ValueError:
...
def handle_sync_results(notifier: notifications.SlackNotifier, data_name: str, remote_name: str, pull: bool,
                        sync_info: SyncInfo, notify_mode: notifications.Mode, log: logging.Logger):
    log.debug("handle_sync_results({}, {}, {}, {})".format(
    ...
    if not notifier:
        ...
        if response.is_error():
            log.error("Error during notify: code: {} message: {}".format(
...
def handle_failed_sync(notifier: notifications.SlackNotifier, data_name: str, remote_name: str, pull: bool,
    ...
    log.debug("handle_failed_sync({}, {}, {}, {})".format(
        data_name, remote_name, notify_mode.name, pull))
    if not notifier:
        ...
        title = "Sync FAILED"
        fields = {"Data": data_name,
                  "Remote": remote_name, "Error": error_message}
        frame_info = getframeinfo(currentframe())
        response = notifier.notify(title, severity, fields=fields, file_name=os.path.basename(frame_info.filename),
                                   line_number=frame_info.lineno, time=datetime.datetime.now().timestamp())
        if response.is_error():
