...
class RsyncSettings:
    def __init__(self, remote_name: str, options: Dict[str, Any] = None):
        ...
        self.options = RsyncOptions(
            **options) if options else RsyncOptions()
...
class ResticCommand:
    def __init__(self, command: str, options: Dict[str, Any] = None, default_options: Dict[str, Any] = None,
    ...
    def run(self, restic_wrapper: ResticWrapper, log: logging.Logger, global_options: Dict[str, Any] = None,
            args: List[str] = None):
        return restic_wrapper.run_command(self.command, log, global_options=global_options, options=self.options,
                                          args=args)
...
class ResticBackupCommand(ResticCommand):
    def __init__(self, options: Dict[str, Any] = None, skip_defaults: bool = None):
        ...
        if not skip_defaults:
            ...
            self.options['tags'] = self.options['tags'] + \
                default_tags if 'tags' in self.options else default_tags
...
class ResticDriver:
    def __init__(self, settings: Dict[str, Any], commands: List[Dict[str, Any]]):
        ...
        self.commands = [ResticCommand.construct(
...
class ResticRepository:
    def __init__(self, password_provider: Dict[str, str], backend: Dict[str, str]):
        ...
        if backend['type'] == 'rest':
            backend['settings']['password_provider'] = PasswordProvider(
                **backend['settings']['password_provider'])
            self.backend = RestBackend(path=backend['path'], env=backend['env'] if 'env' in backend else None,
                                       **backend['settings'])
        else:
            self.backend = RepoConnection(
                path=backend['path'], env=backend['env'] if 'env' in backend else None)
...
class AutoSync:
    def __init__(self, sync_name: str, driver: Dict[str, Any], pre_commands: List[Any] = None, notify: str = None, frequency: str = None,
        ...
        self.healthchecks = hc.Healthcheck(
            **healthchecks) if healthchecks else None


class DataDir:
    def __init__(self, path: str, group: str = None, permissions: str = None,
                 auto_sync: List[Dict[str, Any]] = None, driver_common_settings: List[Dict[str, Any]] = None):
        self.path = path
        self.auto_sync = [AutoSync(**s)
                          for s in auto_sync] if auto_sync else []
        ...
        if driver_common_settings:
            for entry in driver_common_settings:
                if entry['type'] == 'rsync':
                    self.rsync_options = self.rsync_options.mask(
                        RsyncOptions(**entry['settings']))
                if entry['type'] == 'restic':
                    self.restic_options = self.restic_options.mask(
                        ResticGlobalOptions(**entry['settings']))
    ...
    def __init__(self, path: str, no_log: bool, debug: bool):
        ...
        if path and os.path.isfile(path):
            ...
            if 'owned_data' in self._raw:
                self.owned_data = {name: DataDir(
                    **value) for name, value in self._raw['owned_data'].items()}
            if 'stored_data' in self._raw:
                self.stored_data = {name: DataDir(
                    **value) for name, value in self._raw['stored_data'].items()}
            if 'remotes' in self._raw:
                self.remotes = {name: Remote(
                    **value) for name, value in self._raw['remotes'].items()}
            if 'restic_repositories' in self._raw:
                self.restic_repositories = {name: ResticRepository(
                    **value) for name, value in self._raw['restic_repositories'].items()}
